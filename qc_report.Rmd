...
plot_barchart <- function(df, var, ord = NULL) {
  ...
  # Si ord no és NULL, convertim la variable a factor amb aquest ordre
  if (!is.null(ord)) {
...
plot_visits_by_cohort <- function(df) {
  df %>%
    ...
    facet_wrap(~ Cohort, scales = "free_x", ncol = 2) +  # escala lliure a l'eix X per cada faceta
...
missing_data <- missing_summary(df_miss) %>%
  ...
  filter(Perc_missing > 0)  # opcional: només mostrar les variables amb missing
