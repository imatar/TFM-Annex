...
read_tsv_file <- function(file_path) {
  ...
  df <- fread(file_path, select = c("subject_id", "region", "roi_volume_mm3", "age_at_acquisition"), na.strings = c("", "NA"))
...
WRAP_Imaging_MRI_hcv_merged <- rbindlist(lapply(tsv_files, read_tsv_file), use.names = TRUE, fill = TRUE)
setcolorder(WRAP_Imaging_MRI_hcv_merged, c("patient_id", "session_id", "subject_id", "region", "roi_volume_mm3", "age_at_acquisition"))

# ─────────────────────────────────────────────────────────────
# Es guarda el fitxer
output_file <- "path/WRAP_Imaging_MRI_hcv_merged.csv"
write.csv(WRAP_Imaging_MRI_hcv_merged, output_file, row.names = FALSE)